...
// C128 text resolution is 40x25
void putpic(int x, int y, int picture) {			
	//putsprite(spr_or,(x*spritesize),(y*spritesize),sprites + (spritemem * (*ptr++)));			
	if (picture == 0) {
		display[164+y*80+x*2]=0;
		display[164+y*80+x*2+1]=0;
		display[164+y*80+40+x*2]=0;
		display[164+y*80+40+x*2+1]=0;
	} else {
		display[164+y*80+x*2]=1+4*picture-4;				
		display[164+y*80+x*2+1]=1+4*picture-2;				
